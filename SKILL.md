...
- **`create_partner_tbank_cart_payment`** — один рублёвый платёж T-Bank за **несколько** одобренных RUB-продуктов партнёра («рублёвая корзина»). Нужна, когда клиент покупает комбинацию: «2 места в команде + 100 аккаунтов» — вместо нескольких ссылок клиент платит одну сумму, а в кассовом чеке **каждая позиция идёт отдельной строкой** (название из каталога, цена за единицу, количество, сумма строки). Обязательно: `items` — 1..100 позиций `{product_id, quantity}`; `quantity` — целое 1..9999; один продукт — одна позиция, количество складывайте в `quantity`. Опционально у позиции `unit_amount` — цена ОДНОЙ единицы **в копейках** (`85000` = 850 ₽; не передали — берётся цена продукта из каталога; в ответе `amount_rub` показывает получившуюся сумму в рублях). Сумму платежа **не передают** — сервер считает Σ цена × количество (от 1 ₽ до 10 000 000 ₽). Все продукты корзины должны идти через один терминал (одна система налогообложения), иначе `INVALID_INPUT` — такую корзину разбивают на отдельные платежи. Ответ — как у `create_partner_tbank_payment` (`payment_url` + `sbp_url`, отдавайте `sbp_url`) плюс `line_item_count` и `items` ровно в том виде, в каком они ушли в чек. Повтор того же запроса возвращает тот же платёж с `duplicate: true`; нужен отдельный платёж того же состава — передайте новый `client_token`. Рекуррентных списаний в рублях нет — корзина разовая. Требует permission `tbank_payment`.
...
2b. Клиент берёт **несколько** продуктов сразу — вместо шага 2 вызывайте `create_partner_tbank_cart_payment` с `items` (`{product_id, quantity}`, опц. `unit_amount` в копейках). Один платёж, в чеке — строка на позицию. Все продукты должны быть на одном терминале.
